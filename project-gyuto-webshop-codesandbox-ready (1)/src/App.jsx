...
import DATA from './products.json'
  ...
  const PRODUCTS = DATA
  { id: "g-vg10-210-nashiji", name: "Gyuto 210 mm – Nashiji", type: "Gyuto", steel: "VG-10", length: 210, finish: "Nashiji", price: 2490, stock: 18, featured: true },
  { id: "s-vg10-180-tsuchime", name: "Santoku 180 mm – Tsuchime", type: "Santoku", steel: "VG-10", length: 180, finish: "Tsuchime", price: 1690, stock: 25 },
  { id: "g-sg2-240-ham-dam", name: "Gyuto 240 mm – Hamret damask", type: "Gyuto", steel: "SG2/R2", length: 240, finish: "Damask/Tsuchime", price: 4790, stock: 12, featured: true },
  { id: "n-vg10-165-dam", name: "Nakiri 165 mm – Damask", type: "Nakiri", steel: "VG-10", length: 165, finish: "Damask", price: 2290, stock: 16 },
  { id: "k-as-240-kurouchi", name: "Kiritsuke 240 mm – Kurouchi", type: "Kiritsuke", steel: "Aogami Super", length: 240, finish: "Kurouchi", price: 7990, stock: 5 },
  { id: "p-sg2-135", name: "Petty 135 mm – Polert", type: "Petty", steel: "SG2/R2", length: 135, finish: "Polert", price: 2090, stock: 20 },
  { id: "b-sg2-165", name: "Bunka 165 mm – Satin", type: "Bunka", steel: "SG2/R2", length: 165, finish: "Satin", price: 3490, stock: 14 },
  { id: "g-vg10-240-nashiji", name: "Gyuto 240 mm – Nashiji", type: "Gyuto", steel: "VG-10", length: 240, finish: "Nashiji", price: 3190, stock: 10 },
  { id: "y-w2-300-kasumi", name: "Yanagiba 300 mm – Kasumi", type: "Yanagiba", steel: "White #2", length: 300, finish: "Kasumi", price: 6790, stock: 7 },
];
...
function Badge({ children }) {
  return <span className="rounded-full bg-white border border-stone-300 shadow-sm px-3 py-1">{children}</span>;
}
...
function ToggleChip({ active, onClick, children }) {
  return (
    <button onClick={onClick} className={\`text-sm px-3 py-1 rounded-full border \${active ? "bg-stone-900 text-white border-stone-900" : "bg-white border-stone-300 hover:bg-stone-50"}\`}>
      {children}
    </button>
  );
}
...
export default function App() {
  ...
  const filtered = useMemo(() => {
    ...
    switch (sort) {
      ...
      case "length":
        list.sort((a, b) => a.length - b.length); break;
      default:
    ...
    <div className="min-h-screen bg-stone-100 text-stone-900">
      <header className="sticky top-0 z-40 backdrop-blur supports-[backdrop-filter]:bg-stone-100/90 bg-stone-100/80 border-b border-stone-200">
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center gap-4">
          ...
          <div className="hidden md:flex items-center gap-6 text-sm">
            {TYPES.map((t) => (
              <button key={t} className="hover:underline" onClick={() => setTypes((prev) => prev.includes(t) ? prev.filter(x => x!==t) : [...prev, t])}>{t}</button>
            ))}
          </div>
          <div className="ml-auto flex items-center gap-3 w-full md:w-auto">
            ...
            <button onClick={()=>setCartOpen(true)} className="relative rounded-xl border border-stone-300 bg-white px-4 py-2 text-sm hover:bg-stone-50">
              ...
              {Object.keys(cart).length>0 && (
                <span className="ml-2 rounded-full bg-stone-900 text-white text-xs px-2 py-0.5">{Object.values(cart).reduce((n,i)=>n+i.qty,0)}</span>
              )}
            </button>
          </div>
        </div>
        ...
        <aside id="filters" className="space-y-6">
          <Card title="Filtre">
            <div className="space-y-4">
              <div>
                <Label>Ståltype</Label>
                <div className="mt-2 grid grid-cols-2 gap-2">
                  {STEELS.map(s => (
                    <ToggleChip key={s} active={steel.includes(s)} onClick={()=>setSteel(prev=> prev.includes(s)? prev.filter(x=>x!==s): [...prev, s])}>{s}</ToggleChip>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  {FINISHES.map(f => (
                    <ToggleChip key={f} active={finish.includes(f)} onClick={()=>setFinish(prev=> prev.includes(f)? prev.filter(x=>x!==f): [...prev, f])}>{f}</ToggleChip>
                  ))}
                </div>
              </div>
              <div>
                <Label>Bladlengde: {lengths[0]}–{lengths[1]} mm</Label>
                <div className="mt-3 grid grid-cols-2 gap-3 items-center">
                  <input type="range" min={120} max={300} value={lengths[0]} onChange={(e)=>setLengths([Number(e.target.value), lengths[1]])} />
                  <input type="range" min={120} max={300} value={lengths[1]} onChange={(e)=>setLengths([lengths[0], Number(e.target.value)])} />
                </div>
              </div>
              <div>
                <Label>Makspris: {NOK(priceMax)}</Label>
                <input className="mt-2 w-full" type="range" min={800} max={9000} step={50} value={priceMax} onChange={(e)=>setPriceMax(Number(e.target.value))} />
              </div>
              <div>
                <Label>Type</Label>
                <div className="mt-2 flex flex-wrap gap-2">
                  {TYPES.map(t => (
                    <ToggleChip key={t} active={types.includes(t)} onClick={()=>setTypes(prev=> prev.includes(t)? prev.filter(x=>x!==t): [...prev, t])}>{t}</ToggleChip>
                  ))}
                </div>
              </div>
              <div className="pt-2 flex gap-2">
                <button className="px-4 py-2 rounded-xl border border-stone-300" onClick={()=>{setSteel([]); setFinish([]); setTypes([]); setLengths([120,300]); setPriceMax(9000); setQuery(""); setSort("featured");}}>Nullstill</button>
              </div>
            </div>
          </Card>
        </aside>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((p) => (
              <article key={p.id} className="group rounded-2xl bg-white border border-stone-200 shadow-sm hover:shadow-md transition overflow-hidden flex flex-col">
                <div className="aspect-[5/3] bg-gradient-to-b from-stone-100 to-stone-50 grid place-items-center">
                  <KnifeSVG className="w-[88%]" />
                </div>
      ...
      <footer className="mt-16 border-t border-stone-200">
        <div className="mx-auto max-w-7xl px-4 py-10 grid sm:grid-cols-2 lg:grid-cols-4 gap-8 text-sm text-stone-600">
          <div>
            <div className="font-serif text-xl text-stone-900">Project Gyuto</div>
            <p className="mt-2">Presisjon fra Echizen – valgt for norske kjøkken.</p>
          </div>
          <div>
            <div className="font-medium text-stone-900">Kundeservice</div>
            <ul className="mt-2 space-y-1">
              <li>Frakt & retur</li>
              <li>Vedlikeholdsguider</li>
              <li>Garanti</li>
            </ul>
          </div>
          <div>
            <div className="font-medium text-stone-900">Trygghet</div>
            <ul className="mt-2 space-y-1">
              <li>80 dagers åpent kjøp</li>
              <li>Gratis første sliping</li>
              <li>Rask levering 1–3 dager</li>
            </ul>
          </div>
          <div>
            <div className="font-medium text-stone-900">Nyhetsbrev</div>
            <p className="mt-2">Få tips om sliping, nye lanseringer og kampanjer.</p>
            <div className="mt-3 flex gap-2">
              <input placeholder="Din e‑post" className="flex-1 rounded-xl border border-stone-300 bg-white px-3 py-2 text-sm" />
              <button className="rounded-xl bg-stone-900 text-white px-4 py-2 text-sm">Meld meg på</button>
            </div>
          </div>
        </div>
      ...
      <div className={\`fixed inset-0 z-50 \${cartOpen ? "" : "pointer-events-none"}\`} aria-hidden={!cartOpen}>
        ...
        <div className={\`absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl border-l border-stone-200 transition-transform \${cartOpen ? "translate-x-0" : "translate-x-full"}\`}>
          <div className="p-5 flex items-center justify-between border-b">
            <div className="font-medium">Handlekurv</div>
            <button className="text-stone-600" onClick={()=>setCartOpen(false)}>Lukk</button>
          </div>
          <div className="p-5 space-y-4 max-h-[60vh] overflow-auto">
            {Object.keys(cart).length===0 && (
              <p className="text-sm text-stone-600">Kurven er tom. Legg til produkter for å fortsette.</p>
            )}
            {Object.values(cart).map(({ product, qty }) => (
              <div key={product.id} className="flex items-center gap-3">
                <div className="w-20 aspect-[5/3] rounded-lg bg-stone-100 grid place-items-center border"><KnifeSVG className="w-[90%]"/></div>
          ...
          <div className="p-5 border-t mt-auto">
            <div className="flex items-center justify-between text-sm">
              <span>Delsum</span>
              <span className="font-semibold">{NOK(subtotal)}</span>
            </div>
            <p className="mt-1 text-xs text-stone-600">Pris inkl. mva. Frakt beregnes i kassen. Gratis frakt over 1 500 kr.</p>
            <button className="mt-4 w-full rounded-2xl bg-stone-900 text-white px-5 py-3 text-sm disabled:opacity-50" disabled={subtotal===0}>Gå til kassen</button>
          </div>
        </div>
      </div>
    </div>
