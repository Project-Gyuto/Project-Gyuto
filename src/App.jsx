...
function PageLearn() {
  return (
    <div className="prose max-w-3xl mx-auto">
      ...
      <p>Japanske kniver har røtter i sverdsmedtradisjonen. Etter Meiji-perioden ble kunnskapen brukt til kjøkkenkniver i byer som <b>Seki</b>, <b>Sakai</b> og <b>Echizen</b>. Her utviklet man tynnere blader, hardere stål og mer presis herding enn i mye vestlig produksjon.</p>
      <ul>
        ...
        <li><b>Sujihiki/Yanagiba</b> – lange fileterings-/oppskjæringskniver</li>
      </ul>
      <h3>Hvorfor oppleves de skarpere?</h3>
      <ul>
        <li>Hardere stål (ofte 60–65 HRC) holder en tynnere egg lenger.</li>
        <li>Lav slipvinkel (ca. 12–15° per side) og tynne blader gir mindre friksjon.</li>
      </ul>
    </div>
  )
}
  ...
  return (
    <div className="prose max-w-3xl mx-auto">
      ...
      <ul>
        ...
        <li><b>SG2 / R2:</b> 63–65 HRC, pulverstål. Svært lang egg­holdning. (Hatsukokoro, Miyabi)</li>
        <li><b>AUS‑8 / MBS‑26:</b> 58–60 HRC, lett å slipe, tilgivende. (Masahiro, Kanetsugu)</li>
      </ul>
      <h3>Finish</h3>
      <ul>
        <li><b>Damask/Suminagashi:</b> bølget lagmønster</li>
        <li><b>Tsuchime:</b> hamret – slipper råvarer</li>
        <li><b>Nashiji:</b> “pære‑skall” tekstur</li>
        <li><b>Kurouchi:</b> mørk smidd overflate</li>
      </ul>
    </div>
  )
}
  ...
  return (
    <div className="prose max-w-3xl mx-auto">
      ...
      <ul>
        ...
        <li>Oppbevaring i slire, magnetlist eller knivblokk.</li>
      </ul>
    </div>
  )
}
      ...
      <section className="border-b border-stone-200">
        <div className="mx-auto max-w-7xl px-4 py-8 grid md:grid-cols-2 items-center gap-10">
          <div>
            ...
            <p className="mt-3 text-stone-700">Vi kuraterer modeller du kan kjøpe og få levert i Norge, med realistiske priser og tydelig stålguide.</p>
            <div className="mt-4 flex flex-wrap gap-2 text-xs text-stone-600">
              <span className="rounded-full bg-white border border-stone-300 shadow-sm px-3 py-1">Åpent kjøp 30 dager</span>
              <span className="rounded-full bg-white border border-stone-300 shadow-sm px-3 py-1">Gratis første sliping</span>
              <span className="rounded-full bg-white border border-stone-300 shadow-sm px-3 py-1">1–3 dagers levering</span>
            </div>
          </div>
        ...
        <aside className="space-y-6">
          ...
          <div className="rounded-2xl bg-white border border-stone-200 shadow-sm p-4">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {STEELS.map(s => <button key={s} onClick={()=>setSteel(prev=>prev.includes(s)?prev.filter(x=>x!==s):[...prev, s])} className={`text-sm px-3 py-1 rounded-full border ${steel.includes(s)?'bg-stone-900 text-white border-stone-900':'bg-white border-stone-300'}`}>{s}</button>)}
            </div>
          </div>
          <div className="rounded-2xl bg-white border border-stone-200 shadow-sm p-4">
            <div className="text-xs uppercase tracking-wide text-stone-600">Type</div>
            <div className="mt-2 flex flex-wrap gap-2">
              {TYPES.map(t => <button key={t} onClick={()=>setTypes(prev=>prev.includes(t)?prev.filter(x=>x!==t):[...prev, t])} className={`text-sm px-3 py-1 rounded-full border ${types.includes(t)?'bg-stone-900 text-white border-stone-900':'bg-white border-stone-300'}`}>{t}</button>)}
            </div>
          </div>
          <div className="rounded-2xl bg-white border border-stone-200 shadow-sm p-4">
